"""OrchestratorAgent — Tier 0: PM-level decomposition and batch delegation."""
...
class OrchestratorAgent(BaseAgent):
    """Top-tier PM agent. Decomposes requests into parallel subtask batches."""

    def __init__(self, bus: EventBus, provider: str | None = None) -> None:
        super().__init__("ORCHESTRATOR", bus, provider=provider)
    ...
    async def execute(
        ...
        try:
            ...
            # Fallback simulation if LLM unavailable
            response = f"[{self.name}] Orchestrating: {prompt}"
    ...
    async def _run_tool_loop(
        self, initial_prompt: str, max_iterations: int = 6
    ) -> AsyncGenerator[str, None]:
        """Tool loop using delegate_batch only. Falls through to plain stream if no executor."""
